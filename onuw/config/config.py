...
class EnvironmentConfig(Config):
    """
    EnvironmentConfig contains an env_type field to indicate the name of the environment.
    """
...
class AgentConfig(Config):
    """
    AgentConfig contains role and backend fields.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # check if the role field is specified
        if "role" not in self:
            raise ValueError("The role field is not specified")
